...
generate() {
  ...
  $generator_script ${VERBOSE:+'-v'} \
    ...
    --plugin 'laplacian-arch:laplacian-arch.deployment.domain-model-plugin:1.0.0' \
    --plugin 'laplacian-arch:laplacian-arch.api-service.domain-model-plugin:1.0.0' \
    --plugin 'laplacian-tutorial:laplacian-tutorial.domain-model-plugin:0.0.1' \
    --template 'laplacian:laplacian.generator.project-template:1.0.0' \
    --template 'laplacian-arch:laplacian-arch.java-stack-service.project-template:1.0.0' \
    --template 'laplacian-arch:laplacian-arch.datasource.flyway-migration-template:1.0.0' \
    --template 'laplacian-arch:laplacian-arch.api-service.springboot2-template:1.0.0' \
    ...
    --model 'laplacian-tutorial:laplacian-tutorial.domain-model:0.0.1' \
    --model 'laplacian-tutorial:laplacian-tutorial.application-model:0.0.1' \
    $schema_option \
